...
def plot_stats(statistics, ylog=False, view=False, filename='avg_fitness.svg'):
    ...
    '''
    if plt is None:
        warnings.warn("This display is not available due to a missing optimal dependency")
        return
...
def plot_spikes(spikes, view=False, filename=False, title=None):
    '''
    ...
    if title is None:
        plt.title("spiking neuron model")
    else:
        plt.title("spiking neuron model ({0!s})".format(title))

...
from tensorflow.keras.layers import Add, Conv2D, Lambda, LeakyReLU, MaxPool2D, Concatenate
from tensorflow.keras.layers import Input, UpSampling2D, ZeroPadding2D, BatchNormalization

from .utils import broadcast_iou

flags.DEFINE_integer('yolo_max_bboxes', 100, 'maximum number of bboxes per imaage')
...
def yolo_bboxes(pred, anchors, classes):
    ...
    class_probs = tf.sigmod(class_probs)
    pred_bbox = tf.concat((bbox_xy, bbox_wh), axis=-1)

    grid = _meshgrid(grid_size[1], grid_size[0])
    grid = tf.expend_dims(tf.stack(grid, axis=-1), axis=2)
...
def yolo_nms(outputs, anchors, masks, classes):
    ...
    selected_indices, selected_scores = tf.image.non_max_suppression_with_scores(
        ...
        scores_threshold=FLAGS.yolo_score_threshold,
        soft_nms_sigma=0.5
    )
...
def YoloV3(size=None, channels=3, anchors=yolo_anchors, masks=yolo_anchor_masks, classes=80, training=False):
    ...
    boxes_0 = Lambda(lambda x: yolo_bboxes(x, anchors[masks[0]], classes), name='yolo_bboxes_0')(output_0)
    boxes_1 = Lambda(lambda x: yolo_bboxes(x, anchors[masks[1]], classes), name='yolo_bboxes_1')(output_1)
    boxes_2 = Lambda(lambda x: yolo_bboxes(x, anchors[masks[2]], classes), name='yolo_bboxes_2')(output_2)
    outputs = Lambda(lambda x: yolo_nms(x, anchors, masks, classes), name='yolo_nms')(boxes_0[:3], boxes_1[:3], boxes_2[:3])

    return Model(inputs, outputs, name='yolov3')

def YoloV3Tiny(size=None, channels=3, anchors=yolo_anchors, masks=yolo_anchor_masks, classes=80, training=False):
    x = inputs = Input([size, size, channels], name='input')
    x_8, x = DarknetTiney(name='yolo_darknet_tiny')(x)
    x = YoloConvTiny(256, name='yolo_conv_tiny_0')(x)
    output_0 = YoloOutput(256, len(masks[0]), classes, name='yolo_output_tiny_0')(x)
    x = YoloConvTiny(128, name='yolo_conv_tiny_1')(x, x_8)
    output_1 = YoloOutput(128, len(masks[1]), classes, name='yolo_output_tiny_1')(x)
...
def YoloLoss(anchors, classes=80, ignore_thresh=0.5):
    def yolo_loss(y_true, y_pred):
        ...
        pred_box, pred_obj, pred_class, pred_xyhw = yolo_bboxes(y_pred, anchors, classes)
        pred_xy = pred_xyhw[..., 0:2]
        pred_wh = pred_xyhw[..., 2:4]
        ...
        # 6. sum over (batch, grid_x, grid_y, anchors) -> (batch, 1)

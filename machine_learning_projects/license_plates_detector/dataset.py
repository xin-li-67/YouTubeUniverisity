...
class Dataset(object):
    ...
    def load_annotations(self, database_type):
        final_annotations = []
    ...
    def delete_bad_annotations(self, bad_annotation):
        ...
        with tf.device('/cpu:0'):
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    img, bboxes = self.parse_annotaion(annotation)

                    try:
                        label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = self.preprocess_true_boxes(bboxes)
                    except IndexError:
                        exceptions = True
                        self.delete_bad_annotations(annotation)
    ...
    def random_horzontal_flip(self, image, bboxes):
    ...
    def parse_annotaion(self, annotation, mAP='False'):
        ...
        if self.data_aug:
            image, bboxes = self.random_horzontal_flip(np.copy(image), np.copy(bboxes))

...
class PredictView(views.APIView):
    def post(self, request, endpoint_name, format=None):
        ...
        if len(algs) != 1 and algorithm_status != "ab_testing":
            return Response(
                {"status": "Error", "message": "ML algorithm is ambiguous. Please specify the version"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def perform_create(self, serializer):
        try:
            with transaction.atomic():
                ...
                # update status for the first alg
                ...
                # update status for the second alg
    ...
    def post(self, request, ab_test_id, format=None):
        try:
            ...
            if ab_test.ended_at is not None:
                return Response({"message": "AB Test has already finished."})

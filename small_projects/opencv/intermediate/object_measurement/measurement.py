...
while True:
    ...
    imgContours, conts = utilities.getContours(img, minArea=5000, filter=4)
    
    if len(conts) != 0:
        biggest = conts[0][2]
        imgWarp = utilities.warpImg(img, biggest, wP, hP)
        # Use the wraped base to find the contours of the object need to measure
        imgContours2, conts2 = utilities.getContours(imgWarp, minArea=2000, filter=4, cThr=[50,50], draw=False)

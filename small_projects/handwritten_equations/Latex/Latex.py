...
class Latex(object):
    def __init__(self, model_dir=None, mean_train=None, std_train=None, plotting=False, verbose=False):
        ...
        if std_train is None:
            raise ValueError("std_train cannot be empty")
        ...
        self.classifier = tf.estimator.Estimator(model_fn=self.cnn_model_fn, model_dir=self.model_dir)
    ...
    def train(self, train_images, train_labels, steps):
        ...
        """
        train_input_fn = tf.estimator.inputs.numpy_input_fn(x={"x": train_images}, y=train_labels, batch_size=500, num_epochs=None, shuffle=True)
    ...
    def cnn_model_fn(self, features, labels, mode):
        ...
        conv1 = tf.layers.conv2d(inputs=input_layer, filters=32, kernel_size=[7,7], padding="same", activation=tf.nn.relu)
        pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2,2], strides=2)
        conv2 = tf.layers.conv2d(inputs=pool1, filters=64, kernel_size=[7,7], padding="same", activation=tf.nn.relu)
        pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2,2], strides=2)
        conv3 = tf.layers.conv2d(inputs=pool2, filters=128, kernel_size=[7,7], padding="same", activation=tf.nn.relu)
